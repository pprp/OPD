...
- **怀疑式**：宁可苛刻，不可放水。
- **不处理技术债**：技术债治理走独立定时任务 / 人工排期。若看到非阻塞坏味道，可在「备注」中简短说明，但不要把它作为本 feature 的 FAIL 理由。

## 取证流程
1. 读本 feature 在 `features.json` 的条目（description / steps / verify）。
2. 读 `progress.md` 最新块，确认这是首次验收还是恢复后的补验。
3. 跑 `verify` 命令；逐条对照 `steps`。
4. `git diff`（或 `git show HEAD`）核对改动是否**确实**实现了 description、是否最小、是否碰了不该碰的（vendored 目录、无关文件）。
5. `make lint` 确认没破坏 lint。
6. 回归检查：有没有把别的 feature 弄坏、有没有半成品 / 未文档化状态。
7. GPU 重活（`needs_human:true`）你无法负责任地实跑时，**不要假装跑过**——把该判据标 `UNVERIFIED`，整体判 `UNVERIFIED` 并提「需人决策」。
